...
public class ColladaExporter
{
	...
	public static void Export(Stream input, Stream output)
	{
		...
		colladaExporter.AddModel(ModelContentReader.Read(input, out bool _));
		colladaExporter.Save(output);
	}
	...
	{
		XElement[] array = new XElement[model.Bones.Count];
		for (int i = 0; i < array.Length; i++)
		{
			ModelBoneData modelBoneData = model.Bones[i];
			array[i] = GetNode(modelBoneData.Name, modelBoneData.Transform);
		}
		for (int j = 0; j < array.Length; j++)
		{
			ModelBoneData modelBoneData2 = model.Bones[j];
		...
		{
			XElement geometry = GetGeometry(model, mesh);
	...
	private XElement GetGeometry(ModelData model, ModelMeshData data)
	{
		...
		_ = model.Bones[data.ParentBoneIndex];
		...
		using (EngineBinaryReader engineBinaryReader = new EngineBinaryReader(new MemoryStream(array)))
		{
			foreach (VertexElement vertexElement in vertexDeclaration.VertexElements)
			{
				if (vertexElement.Semantic.StartsWith("POSITION"))
				{
					...
					{
						engineBinaryReader.BaseStream.Position = vertexDeclaration.VertexStride * j + vertexElement.Offset;
						Vector3 vector = new Vector3(engineBinaryReader.ReadSingle(), engineBinaryReader.ReadSingle(), engineBinaryReader.ReadSingle());
				...
				else if (vertexElement.Semantic.StartsWith("NORMAL"))
				{
					...
					{
						engineBinaryReader.BaseStream.Position = vertexDeclaration.VertexStride * k + vertexElement.Offset;
						Vector3 vector2 = new Vector3(engineBinaryReader.ReadSingle(), engineBinaryReader.ReadSingle(), engineBinaryReader.ReadSingle());
				...
				else if (vertexElement.Semantic.StartsWith("TEXCOORD"))
				{
					...
					{
						engineBinaryReader.BaseStream.Position = vertexDeclaration.VertexStride * l + vertexElement.Offset;
						array2[l * 2] = engineBinaryReader.ReadSingle();
						array2[l * 2 + 1] = 1f - engineBinaryReader.ReadSingle();
						if (l % 3 == 2)
						{
							array3[l * 3 - 4] = l - 2;
							array3[l * 3 - 1] = l;
		...
		XElement source3 = (XElement)(array6[2] = GetSourceArray(name, "-mesh-map", string.Join(" ", array2.Select((float f) => f.ToString("R"))), array2.Length, 2, STParam()));
